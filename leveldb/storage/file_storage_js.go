...
func fsSyncDir(dirname string) error {
	// fs doesn't support syncDir directly so we need to implement it
	// manually by walking through the directory.
	names, err := fsReaddirnames(dirname, 0)
	if err != nil {
		return err
	}
...
func (fl *fsFileLock) release() error {
	if fl.readOnly {
		locksMu.Lock()
...
func newFileLock(path string, readOnly bool) (fl fileLock, err error) {
	...
	return &fsFileLock{file: file, path: path}, nil
}
